...
namespace bad
{
	...
	public:
		...
		~Kapsam()
		{
			for (auto i = tanýmNesneleri.rbegin(); i != tanýmNesneleri.rend(); i++)
				delete* i;
			for (auto i = altKapsamlar.rbegin(); i != altKapsamlar.rend(); i++)
				delete* i;
		}

		ÝþlevKapsamý* iþlevKapsamýAç()
		{
			ÝþlevKapsamý* kapsam = new ÝþlevKapsamý();
			altKapsamlar.push_back(kapsam);
			return kapsam;
		}

		BütünKapsamý* bütünKapsamýAç()
		{
			BütünKapsamý* kapsam = new BütünKapsamý();
			altKapsamlar.push_back(kapsam);
			return kapsam;
		}

		GövdeKapsamý* gövdeKapsamýAç()
		{
			GövdeKapsamý* kapsam = new GövdeKapsamý();
			altKapsamlar.push_back(kapsam);
			return kapsam;
		}

		YazýlýmKapsamý* yazýlýmKapsamýAç()
		{
			YazýlýmKapsamý* kapsam = new YazýlýmKapsamý();
			altKapsamlar.push_back(kapsam);
			return kapsam;
		}
	...
	public:
		...
		const KaynakDosyasý* bulunduðuDosya;
		...
		KaynakKonumu(
			...
			const KaynakDosyasý* bulunduðuDosya)
			:
			baþlangýçSatýrý(new int(baþlangýçSatýrý)),
			baþlangýçSütunu(new int(baþlangýçSütunu)),
			bitiþSatýrý(new int(bitiþSatýrý)),
			bitiþSütunu(new int(bitiþSütunu)),
			bulunduðuDosya(bulunduðuDosya)
		{}

		~KaynakKonumu()
		{
			delete baþlangýçSatýrý;
			delete baþlangýçSütunu;
			delete bitiþSatýrý;
			delete bulunduðuDosya;
		}
	};
	...
	public:
		...
		~DerlemeBirimi()
		{
			for (auto i = parçalarý.begin(); i != parçalarý.end(); i++)
				delete* i;
			delete açtýðýKapsam;
		}
	};
	...
	public:
		...
		const KaynakDosyasý* gönderdiðiDosya;
